                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <?php if (!empty($loading['bill_of_lading_file'])): ?>
                                                <small class="text-muted">
                                                    <?= __('loadings.current_file') ?>: <a href="/<?= $loading['bill_of_lading_file'] ?>" target="_blank"><?= __('loadings.view_file') ?></a>
                                                </small>
                                            <?php endif; ?>
                                        </div>
                                    </div>
...
document.getElementById('loading-form').addEventListener('submit', function(e) {
    ...
    // التحقق من رقم التحميل
    if (!loadingNumber) {
        e.preventDefault();
        alert('<?= __('messages.loading_no_required') ?>');
        return false;
    }
    ...
    if (bolFile && bolFile.files.length > 0) {
        const file = bolFile.files[0];
        const allowedTypes = ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png'];
        
        if (!allowedTypes.includes(file.type)) {
            e.preventDefault();
            alert('نوع الملف غير مسموح. يُسمح فقط بـ PDF, JPG, PNG');
            return false;
        }
...
// تحديث تاريخ BOL تلقائياً عند تغيير الحالة إلى "صدرت"
document.getElementById('bol_status').addEventListener('change', function() {
    if (this.value === 'issued' && !document.getElementById('bol_date').value) {
        document.getElementById('bol_date').value = new Date().toISOString().split('T')[0];
    }
});

// عرض مؤشر التحميل أثناء المزامنة
function showSyncLoading() {
    const btn = event.target;
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span> <?= __('messages.sync_in_progress') ?>';
}
</script>
