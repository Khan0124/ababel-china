...
// Get related transaction for payment info
$stmt = $db->query("SELECT t.*, c.name as client_name, c.client_code FROM transactions t 
                    LEFT JOIN clients c ON t.client_id = c.id 
                    WHERE t.loading_no = ? AND t.status = 'approved'", [$loading['loading_no']]);
$relatedTransaction = $stmt->fetch();

// Calculate total outstanding balance if transaction exists
$totalOutstanding = 0;
$paymentInfo = [];
if ($relatedTransaction) {
    $totalOutstanding = getOutstandingAmount($relatedTransaction['balance_rmb'] ?? 0) + 
                       getOutstandingAmount($relatedTransaction['balance_usd'] ?? 0) + 
                       getOutstandingAmount($relatedTransaction['balance_sdg'] ?? 0) + 
                       getOutstandingAmount($relatedTransaction['balance_aed'] ?? 0);
    
    $paymentInfo = [
        'total_paid_rmb' => $relatedTransaction['payment_rmb'] ?? 0,
        'total_paid_usd' => $relatedTransaction['payment_usd'] ?? 0,
        'total_paid_sdg' => $relatedTransaction['payment_sdg'] ?? 0,
        'balance_rmb' => $relatedTransaction['balance_rmb'] ?? 0,
        'balance_usd' => $relatedTransaction['balance_usd'] ?? 0,
        'balance_sdg' => $relatedTransaction['balance_sdg'] ?? 0,
        'balance_aed' => $relatedTransaction['balance_aed'] ?? 0,
    ];
}
?>
        ...
        <div class="btn-group">
            ...
            <?php elseif ($loading['bol_number']): ?>
                <span class="btn btn-success disabled">
                    <i class="bi bi-check-circle"></i> BOL: <?= h($loading['bol_number'] ?? '') ?>
                </span>
            <?php endif; ?>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <p class="mb-0"><?= nl2br(h($loading['notes'] ?? '')) ?></p>
                        </div>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="card mb-4">
                <div class="card-header bg-success text-white">
                    <h5 class="mb-0">
                        ...
                        <?= __('transactions.payment_status') ?>
                    </h5>
                </div>
                <div class="card-body">
                    <!-- Payment Summary -->
                    <table class="table table-sm">
                        <?php if ($paymentInfo['total_paid_rmb'] > 0): ?>
                        <tr>
                            <td><?= __('transactions.paid') ?> (RMB)</td>
                            <td class="text-end text-success">¥<?= number_format($paymentInfo['total_paid_rmb'], 2) ?></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if ($paymentInfo['total_paid_usd'] > 0): ?>
                        <tr>
                            <td><?= __('transactions.paid') ?> (USD)</td>
                            <td class="text-end text-success">$<?= number_format($paymentInfo['total_paid_usd'], 2) ?></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if ($paymentInfo['total_paid_sdg'] > 0): ?>
                        <tr>
                            <td><?= __('transactions.paid') ?> (SDG)</td>
                            <td class="text-end text-success">SDG <?= number_format($paymentInfo['total_paid_sdg'], 2) ?></td>
                        </tr>
                        <?php endif; ?>
                        
                        <!-- Outstanding Balances -->
                        <?php if (getOutstandingAmount($paymentInfo['balance_rmb']) > 0): ?>
                        <tr class="border-top">
                            <td><strong><?= __('transactions.outstanding') ?> (RMB)</strong></td>
                            <td class="text-end text-danger"><strong><?= formatBalance($paymentInfo['balance_rmb'], 'RMB') ?></strong></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if (getOutstandingAmount($paymentInfo['balance_usd']) > 0): ?>
                        <tr>
                            <td><strong><?= __('transactions.outstanding') ?> (USD)</strong></td>
                            <td class="text-end text-danger"><strong><?= formatBalance($paymentInfo['balance_usd'], 'USD') ?></strong></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if (getOutstandingAmount($paymentInfo['balance_sdg']) > 0): ?>
                        <tr>
                            <td><strong><?= __('transactions.outstanding') ?> (SDG)</strong></td>
                            <td class="text-end text-danger"><strong><?= formatBalance($paymentInfo['balance_sdg'], 'SDG') ?></strong></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if (getOutstandingAmount($paymentInfo['balance_aed']) > 0): ?>
                        <tr>
                            <td><strong><?= __('transactions.outstanding') ?> (AED)</strong></td>
                            <td class="text-end text-danger"><strong><?= formatBalance($paymentInfo['balance_aed'], 'AED') ?></strong></td>
                        </tr>
                        <?php endif; ?>
                    </table>
                    ...
                    <div class="d-grid gap-2 mt-3">
                        <a href="/clients/statement/<?= $relatedTransaction['client_id'] ?>" class="btn btn-primary">
                            <i class="bi bi-credit-card me-1"></i>
                            <?= __('transactions.make_payment') ?>
                        </a>
                        <a href="/transactions/view/<?= $relatedTransaction['id'] ?>" class="btn btn-outline-info">
                            <i class="bi bi-eye me-1"></i>
                            <?= __('transactions.view_transaction') ?>
                        </a>
                    </div>
                    <?php else: ?>
                    <div class="alert alert-success mb-0">
                        <i class="bi bi-check-circle me-2"></i>
                        <?= __('transactions.fully_paid') ?>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            <?php elseif ($loading['status'] !== 'cancelled'): ?>
            <!-- No Transaction Yet -->
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="alert alert-warning mb-3">
                        ...
                        <?= __('transactions.no_transaction_created') ?>
                    </div>
                    <div class="d-grid">
                        <a href="/transactions/create?loading_no=<?= urlencode($loading['loading_no']) ?>" class="btn btn-primary">
                            <i class="bi bi-plus-circle me-1"></i>
                            <?= __('transactions.create_transaction') ?>
                        </a>
                    </div>
                </div>
            </div>
