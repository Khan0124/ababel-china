...
class Transaction extends Model
{
    ...
    {
        // Ensure limit is an integer and directly embed it (safe since it's not user input)
        $limit = intval($limit);
        $sql = "
            SELECT t.*, 
                   c.name as client_name,
    ...
    {
        // في النظام المحاسبي الاحترافي:
        // - المبيعات (أنواع 1,2,4,5): تزيد الدين على العميل (رصيد سالب)
        // - المدفوعات (نوع 3): تقلل الدين (رصيد موجب)
        // الرصيد النهائي = المدفوعات - المبيعات
        // رصيد سالب = دين على العميل
        // رصيد موجب = رصيد فائض للعميل
        
        $sql = "
            UPDATE clients c
            SET 
                balance_rmb = (
                    -- حساب المدفوعات المستلمة (موجب)
                    ...
                    -- طرح المبيعات والمصاريف (المبالغ المستحقة على العميل)
                    SELECT COALESCE(SUM(CASE 
                ...
                balance_usd = (
                    -- حساب المدفوعات المستلمة USD (موجب)
                    SELECT COALESCE(SUM(CASE 
                ...
                balance_sdg = (
                    -- حساب المدفوعات المستلمة SDG (موجب)
                    SELECT COALESCE(SUM(CASE 
                ...
                balance_aed = (
                    -- حساب المدفوعات المستلمة AED (موجب)
                    SELECT COALESCE(SUM(CASE 
...
public function getByLoadingId($loadingId)
{
    $sql = "SELECT t.*, 
            ...
            COALESCE(c.name_ar, '') as client_name_ar
            FROM {$this->table} t
 ...
 * النظام المحاسبي الاحترافي: المدفوعات - المبيعات = الرصيد النهائي
 */
public function getClientBalancesByCurrency($clientId)
{
    $sql = "SELECT 
            -- الرصيد بالـ RMB: المدفوعات - المبيعات
            (
                SELECT COALESCE(SUM(CASE 
                    WHEN tt.type = 'income' THEN t.payment_rmb
                    ELSE 0 
            ...
            -- الرصيد بالـ USD: المدفوعات - المبيعات
            (
                SELECT COALESCE(SUM(CASE 
                    WHEN tt.type = 'income' THEN t.payment_usd
                    ELSE 0 
            ...
            (
                ...
                JOIN transaction_types tt ON t.transaction_type_id = tt.id
                WHERE t.client_id = ? AND t.status = 'approved'
            ) as total_balance_aed";
    
    $stmt = $this->db->query($sql, [$clientId, $clientId, $clientId, $clientId, $clientId, $clientId, $clientId, $clientId]);
    return $stmt->fetch();
}
