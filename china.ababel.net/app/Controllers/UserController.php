...
class UserController extends Controller
{
    ...
    protected function store()
    {
        try {
            ...
            // 2025-01-11: تسجيل إضافة مستخدم جديد
            $this->logActivity('user_created', $userId, 'تم إنشاء مستخدم جديد: ' . $_POST['username']);
    ...
    public function update($id)
    {
        try {
            ...
            // 2025-01-11: تسجيل تحديث بيانات المستخدم
            $updatedFields = [];
            if (!empty($_POST['password'])) $updatedFields[] = 'كلمة المرور';
            $updatedFields[] = 'البيانات الشخصية';
            $this->logActivity('user_updated', $id, 'تم تحديث بيانات المستخدم: ' . implode(', ', $updatedFields));
    ...
    public function delete($id)
    {
        try {
            ...
            // 2025-01-11: تسجيل حذف المستخدم
            $this->logActivity('user_deleted', $id, 'تم حذف المستخدم: ' . $user['username']);
    ...
    public function toggleStatus($id)
    {
        try {
            ...
            // 2025-01-11: تسجيل تغيير حالة المستخدم
            $statusArabic = $newStatus === 'active' ? 'نشط' : 'غير نشط';
            $this->logActivity('user_status_changed', $id, "تم تغيير حالة المستخدم إلى: $statusArabic");
     ...
     * 2025-01-11: تم تحديث دالة النشاطات لعرض البيانات الحقيقية من جدول audit_log
     */
    public function activity($id)
    {
        try {
            ...
            // Get user activities with proper mapping for display
            $sql = "SELECT 
                        action, 
                        new_values as description, 
                        ip_address, 
                        user_agent, 
                        created_at,
                        CASE 
                            WHEN action LIKE '%login%' THEN 'login'
                            WHEN action LIKE '%error%' OR action LIKE '%failed%' THEN 'error'
            ...
            // Log that someone viewed this user's activity
            $this->logActivity('activity_viewed', $id, 'عرض سجل النشاطات للمستخدم: ' . $user['username']);
    ...
    /**
     * Log user activity
     * 2025-01-11: إصلاح تسجيل الأنشطة ليتطابق مع بنية جدول audit_log
     */
