   ...
   public function syncBolToPortSudan($loadingId, $bolData)
{
    try {
        ...
        $syncData = [
            ...
            'bill_of_lading_date' => $bolData['bill_of_lading_date'] ?? null,
            'bill_of_lading_file' => $bolData['bill_of_lading_file'] ?? null
        ];
        
        // إرسال البيانات إلى Port Sudan
        $response = $this->makeApiCall([
            'action' => 'update_bol',
            'data' => $syncData
        ], 'POST');
        ...
        if (!isset($response['success']) || !$response['success']) {
            throw new \Exception($response['message'] ?? 'BOL sync failed');
        }
        
        return true;
        
    } catch (\Exception $e) {
        error_log("BOL sync error for loading $loadingId: " . $e->getMessage());
        return false;
    }
}
