...
<div class="col-md-12 p-4">
    <div class="d-flex justify-content-between align-items-center mb-4 no-print">
        ...
        <div>
            <?php 
            // Check if this is a claim with outstanding balance
            $hasBalance = ($transaction['balance_rmb'] > 0 || 
                          $transaction['balance_usd'] > 0 || 
                          $transaction['balance_sdg'] > 0 || 
                          $transaction['balance_aed'] > 0);
            
            if ($hasBalance && $transaction['transaction_type_id'] == 1 && $transaction['status'] != 'cancelled'): 
            ?>
            <a href="/transactions/partial-payment/<?= $transaction['id'] ?>" class="btn btn-primary">
                <i class="bi bi-cash-coin"></i> <?= __('transactions.make_payment') ?>
            </a>
            <?php endif; ?>
        ...
        <div class="col-md-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="mb-0"><?= nl2br(h($transaction['description'] ?? '')) ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <!-- SDG Section -->
                    <?php if ($transaction['payment_sdg'] > 0 || $transaction['balance_sdg'] > 0): ?>
                    <h6 class="text-muted">SDG</h6>
                    <div class="table-responsive mb-3">
                        <table class="table table-sm">
                            <tr>
                                <td><?= __('transactions.payment') ?></td>
                                <td class="text-end text-success"><?= number_format($transaction['payment_sdg'], 2) ?></td>
                            </tr>
                            <tr class="table-primary">
                                <th><?= __('balance') ?></th>
                                <th class="text-end"><?= number_format($transaction['balance_sdg'], 2) ?></th>
                            </tr>
                        </table>
                    </div>
                    <?php endif; ?>
                    ...
                    <!-- AED Section -->
                    <?php if ($transaction['payment_aed'] > 0 || $transaction['balance_aed'] > 0): ?>
                    <h6 class="text-muted">AED</h6>
                    <div class="table-responsive mb-3">
                        <table class="table table-sm">
                            <tr>
                                <td><?= __('transactions.payment') ?></td>
                                <td class="text-end text-success"><?= number_format($transaction['payment_aed'], 2) ?></td>
                            </tr>
                            <tr class="table-primary">
                                <th><?= __('balance') ?></th>
                                <th class="text-end"><?= number_format($transaction['balance_aed'], 2) ?></th>
                            </tr>
                        </table>
                    </div>
                    <?php endif; ?>
